 ...
 * @brief DFRobot's LCD adapted for ESP32-C3 using ESP-IDF
 */
...
void DFRobot_LCD::clear()
{
    ...
    vTaskDelay(pdMS_TO_TICKS(2));  // Delay for LCD clear command
}

void DFRobot_LCD::home()
{
    command(LCD_RETURNHOME);         
    vTaskDelay(pdMS_TO_TICKS(2));  // Delay for LCD home command
}
...
{
    i2c_master_write_to_device(I2C_MASTER_NUM, _lcdAddr, data, len, pdMS_TO_TICKS(I2C_TIMEOUT_MS));
}

void DFRobot_LCD::setReg(uint8_t addr, uint8_t data)
{
    uint8_t write_buf[2] = { addr, data };
    i2c_master_write_to_device(I2C_MASTER_NUM, _RGBAddr, write_buf, 2, pdMS_TO_TICKS(I2C_TIMEOUT_MS));
}
